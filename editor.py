...
def force_sync_working_to_data():
    """Forces the current working resort into the main data object."""
...
def sanitize_loaded_data(data: Dict[str, Any]) -> Dict[str, Any]:
    """Fixes legacy data structures immediately upon load."""
...
def create_download_button_v2(data: Dict[str, Any]):
    ...
    with st.sidebar.expander("💾 Save & Download", expanded=False):
        ...
        st.download_button("⬇️ DOWNLOAD JSON FILE", data=json_data, file_name=filename, mime="application/json", key="download_v2_btn", type="primary", use_container_width=True)
...
def handle_resort_switch_v2(data: Dict[str, Any]):
    ...
    if prev and prev != current:
        if prev in st.session_state.working_resorts:
            ...
            if idx is not None: data["resorts"][idx] = copy.deepcopy(st.session_state.working_resorts[prev])
...
def edit_resort_basics(working: Dict[str, Any], resort_id: str):
    ...
    with c1: st.text_input("Display Name", value=working.get("display_name", ""), key=rk(resort_id, "dn"), on_change=on_basic_info_change, args=(rk(resort_id, "dn"), "display_name"))
    with c2: st.text_input("Code", value=working.get("code", ""), key=rk(resort_id, "cd"), on_change=on_basic_info_change, args=(rk(resort_id, "cd"), "code"))
    ...
    c3, c4 = st.columns(2)
    with c3: st.text_input("Timezone", value=working.get("timezone", "UTC"), key=rk(resort_id, "tz"), on_change=on_basic_info_change, args=(rk(resort_id, "tz"), "timezone"))
    with c4: st.text_area("Address", value=working.get("address", ""), key=rk(resort_id, "ad"), height=100, on_change=on_basic_info_change, args=(rk(resort_id, "ad"), "address"))
...
def render_global_settings_v2(data: Dict[str, Any], years: List[str]):
    ...
    with c2: render_global_holidays_editor(data, years)
...
def render_global_holidays_editor(data: Dict[str, Any], years: List[str]):
    ...
    if holidays:
        for idx, h in enumerate(holidays):
            ...
            with c_a: st.text(f"{d_str} - {n_str}")
            with c_b: 
                if st.button("🗑️", key=f"gh_del_{sel_year}_{idx}"):
                    holidays.pop(idx)
                    st.session_state.last_save_time = datetime.now()
                    st.rerun()
    else: st.caption("No global holidays for this year.")
...
def render_reference_points_editor_v2(working: Dict[str, Any], years: List[str], resort_id: str):
    ...
    # 1. Gather ALL room types from Seasons AND Holidays
    all_rooms = set()
    ...
    for s_idx, season in enumerate(base_year_obj.get("seasons", [])):
        with st.expander(f"🏖️ {season.get('name')}", expanded=True):
            ...
            for key, cat in dc.items():
                # --- VISUALIZE DAY PATTERN ---
                pat = cat.get("day_pattern", [])
                pat_str = ", ".join(pat) if pat else "No Days Assigned"
                st.markdown(f"**📅 {key} ({pat_str})**")
                
                # --- EDIT PATTERN ---
                with st.popover(f"Edit Days ({key})"):
                     new_pat = st.multiselect("Days", WEEKDAYS, default=pat, key=rk(resort_id, "pat", base_year, s_idx, key))
                     if st.button("Update Days", key=rk(resort_id, "pat_btn", base_year, s_idx, key)):
                         cat["day_pattern"] = new_pat
                         save_data_cb()
                         st.rerun()
...
def render_resort_summary_v2(working: Dict[str, Any], years: List[str]):
    ...
    # 1. FIXED: Correctly gather Room Types from Seasons AND Holidays
    room_types = set()
    for y_obj in working.get("years", {}).values():
        for s in y_obj.get("seasons", []):
            for c in s.get("day_categories", {}).values():
        ...
        for h in y_obj.get("holidays", []):
            room_types.update(h.get("room_points", {}).keys())
    ...
    for h in ref_year_data.get("holidays", []):
        ...
        # FIXED: Ensure non-zero values display correctly
        r.update({rt: (rp.get(rt) if (rp.get(rt) is not None and int(rp.get(rt) or 0) > 0) else "—") for rt in room_types})
        rows.append(r)
