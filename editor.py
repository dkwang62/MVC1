...
def render_single_season_v2(
    ...
    # Build new periods and directly assign without comparison
    new_periods = []
    ...
    # ALWAYS assign - let Streamlit handle the update cycle
    ...
    if not base_holidays:
        ...
        for h_idx, h in enumerate(base_holidays):
            ...
            with st.expander(f"🎊 {disp_name}", expanded=False):
                ...
                editor_key = rk(resort_id, "holiday_master_rp_editor", base_year, h_idx)
                state_key = f"{editor_key}_state"
                
                if state_key not in st.session_state:
                    st.session_state[state_key] = df_pts.copy()
