...
# =============================================
# West to East Sorting – Regional groups, then West→East within region
# =============================================
...
REGION_US_CARIBBEAN = 0     # USA + Canada + Caribbean
REGION_MEX_CENTRAL = 1      # Mexico + Costa Rica
REGION_EUROPE = 2
REGION_ASIA_AU = 3
REGION_FALLBACK = 99        # Everything else / unknown
...
def _extract_country_or_region_code(resort: Dict[str, Any]) -> str:
    ...
    We look at several possible fields to be robust against schema differences.
    """
...
def get_region_priority(resort: Dict[str, Any]) -> int:
    ...
    - If timezone clearly says Europe or Asia/Australia, TRUST the timezone.
      (So Bali: tz='Asia/Singapore' → Asia/AU, even though code='ID'
       also exists as a US state code.)
...
def sort_resorts_west_to_east(resorts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Sort resorts so the dropdown flows TOP→BOTTOM as:
