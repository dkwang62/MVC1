...
# Corrected import path to avoid circularity
...
class MVCRepository:
    ...
    def get_resort(self, resort_name: str) -> Optional[ResortData]:
        ...
        # FIX: Ensure 'name' uses plain resort_name (no emojis) for chart titles
    ...
    def calculate_breakdown(
        ...
        while i < nights:
            ...
            if holiday and holiday.name not in processed_holidays:
                ...
                mul = 1.0
                if is_owner:
                    mul = owner_config.get("disc_mul", 1.0) if owner_config else 1.0
                else:
                ...
                c = math.ceil(eff * owner_config.get("cap_rate", 0.0)) if is_owner and owner_config and owner_config.get("inc_c") else 0
                dp = math.ceil(eff * owner_config.get("dep_rate", 0.0)) if is_owner and owner_config and owner_config.get("inc_d") else 0
                cost = m + c + dp if is_owner else math.ceil(eff * rate)
        ...
        if is_owner and owner_config:
            tot_m = math.ceil(tot_eff_pts * rate)
            tot_c = math.ceil(tot_eff_pts * owner_config.get("cap_rate", 0.0)) if owner_config.get("inc_c") else 0
            tot_d = math.ceil(tot_eff_pts * owner_config.get("dep_rate", 0.0)) if owner_config.get("inc_d") else 0
...
# [Note: get_all_room_types_for_resort, build_season_cost_table, and main/run blocks as previously defined]
