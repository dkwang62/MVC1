...
# ----------------------------------------------------------------------
# Core data generation (unchanged)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# OWNER: Single + Comparison (CLEANED)
# ----------------------------------------------------------------------
...
def compare_owner(resort, rooms, checkin, nights, rate, disc_lvl,
    ...
    data_rows = []
    chart_rows = []
    totals = {r: {"pts": 0, "cost": 0} for r in rooms}
    ...
    for i in range(nights):
        ...
        for room in rooms:
            pts = entry.get(room, 0)
            eff_pts, _ = apply_discount(pts, disc_lvl=disc_lvl)
            totals[room]["pts"] += eff_pts

            maint = eff_pts * rate if inc_maint else 0
            cap = eff_pts * cap_per_pt * coc if inc_cap else 0
            dep = eff_pts * (cap_per_pt - salvage) / life if inc_dep else 0
            total_night = maint + cap + dep
            totals[room]["cost"] += total_night
    ...
    for r in rooms:
        total_row[r] = f"${totals[r]['cost']:,.0f}"
    ...
    holiday_chart = [{"Holiday": h, "Room Type": room, "CostValue": cost}
...
def compare_renter(resort, rooms, checkin, nights, rate, discount):
    ...
    holiday_chart = [{"Holiday": h, "Room Type": room, "RentValue": cost}
                     for room in rooms for h, cost in holiday_totals[room].items()]
    holiday_df = pd.DataFrame(holiday_chart)
    chart_df = pd.DataFrame(chart_rows)
...
if st.button("Calculate", type="primary"):
    ...
    if user_mode == "Owner":
        ...
        # Hide redundant Total column by selecting only needed ones
        display_cols = ["Date", "Day", "Points", "Maintenance", "Capital", "Depreciation"] + ([room] if room in df.columns else [])
        st.dataframe(df[display_cols], use_container_width=True)
        
        if disc_lvl > 0:
            st.success(f"{disc_lvl}% Discount Applied → {pts:,} points used | Total Cost: ${total_cost:,.0f}")
        else:
        ...
        if compare:
            ...
            if not chart_df.empty:
                fig = px.bar(chart_df, x="Day", y="CostValue", color="Room Type", barmode="group",
                             text="CostValue", height=600, category_orders={"Day": ["Fri","Sat","Sun","Mon","Tue","Wed","Thu"]})
