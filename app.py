...
ROOM_VIEW_LEGEND = {**{k: v for k, v in [
...
# ----------------------------------------------------------------------
# Core Functions (unchanged + rock solid)
# ----------------------------------------------------------------------
...
def gantt_chart(resort: str, year: int):
    ...
    if df.empty:
        df = pd.DataFrame({"Task": ["No Data"], "Start": [datetime(year,1,1).date()],
    ...
    fig.update_layout(showlegend=True, legend_title="Type", font=dict(family="Arial", size=12))
    return fig
...
# ----------------------------------------------------------------------
# RENTER & OWNER (clean charts like your image)
# ----------------------------------------------------------------------
...
def compare_renter(resort, rooms, checkin, nights, rate, discount):
    ...
    for i in range(nights):
        ...
        for room in rooms:
            ...
            rent = math.ceil(eff_pts * rate)
            totals[room] += rent
            chart_rows.append({"Day": d.strftime("%a"), "Room Type": room, "RentValue": rent, "Label": rent})
    df = pd.DataFrame(chart_rows)
    return df, totals, applied

# ----------------------------------------------------------------------
# UI - EXACTLY LIKE YOUR IMAGE
# ----------------------------------------------------------------------
st.set_page_config(page_title="Marriott Cost Calculator", layout="wide")
st.title("Marriott Vacation Club Cost Calculator")

with st.expander("How It Works"):
    st.markdown("**Renter Mode:** Compare nightly rental costs\n**Owner Mode:** Full ownership cost breakdown")

user_mode = st.sidebar.selectbox("Mode", ["Renter", "Owner"])
checkin = st.date_input("Check-in", value=datetime(2026,6,12).date())
nights = st.number_input("Nights", 1, 30, 7, step=1)
...
with st.sidebar:
    ...
    elif "Custom" in opt: rate = st.number_input("Rate $/pt", 0.3, 2.0, 0.60, 0.01)

resorts = st.multiselect("Resort", data["resorts_list"], default=[data["resorts_list"][0]], max_selections=1)
resort = resorts[0]
...
if st.button("Calculate", type="primary"):
    ...
    if user_mode == "Renter":
        ...
        msg = f"**Total: ${total_rent:,}** ({total_pts:,} pts)"
        if applied:
            pct = "30%" if discount == "within_60_days" else "25%"
            msg = f"**{pct} Discount** → " + msg
        st.success(msg)
        ...
        if compare:
            ...
            for i, room in enumerate(all_rooms):
                room_data = chart_df[chart_df["Room Type"] == room]
                fig.add_trace(go.Bar(
                    x=room_data["Day"],
                    y=room_data["RentValue"],
                    name=room,
                    marker_color=colors[i % len(colors)],
                    text=room_data["Label"],
                    textposition="inside",
                    textfont=dict(color="white", size=14, family="Arial Black"),
                    hovertemplate=f"<b>{room}</b><br>%{{x}}<br>${{y:,}}<extra></extra>"
                ))
            fig.update_layout(
                barmode="group",
                title="Daily Rental Comparison",
                xaxis=dict(title="Day", categoryorder="array", categoryarray=["Fri","Sat","Sun","Mon","Tue","Wed","Thu"]),
                yaxis=dict(title="Rent Value"),
