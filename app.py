...
# ----------------------------------------------------------------------
# Gantt Chart - RESTORED TO app(MVC).py PERFECTION
# ----------------------------------------------------------------------
def create_gantt_chart(resort: str, year: int):
    rows = []
    ys = str(year)
    ...
    colors = {t: {"Holiday": "rgb(255,99,71)", "Low Season": "rgb(135,206,250)",
                 ...
                 "Shoulder": "rgb(50,205,50)", "Info": "rgb(128,128,128)"}.get(t, "rgb(169,169,169)")
...
def renter_breakdown(resort, room, checkin, nights, rate, discount):
    ...
    for i in range(nights):
        ...
        row = {"Date": fmt_date(d), "Day": d.strftime("%a"), "Points": eff_pts, room: f"${rent:,}"}
...
# FIXED: df is now defined in both branches
